...
function Probe({ trial, Before, After, done }) {
  ...
  useLayoutEffect(() => {
    ...
    trial.metrics.topicsAtFirstCommit = trial.metrics.mountedIds.size;
    const timer = setTimeout(() => done(listRef.current), OBSERVE_MS);
    return () => clearTimeout(timer);
  }, [done, trial]);
  return (
    <View style={styles.scene}>
      <Component ref={listRef} topics={trial.topics} metrics={trial.metrics} />
    </View>
  );
}
  ...
  useEffect(() => {
    ...
    const runOne = async (variant, size, round, warmup) => {
      ...
      const topics = Array.from({ length: size }, (_, index) => ({
        ...
        content: `모임 발제 내용 ${index + 1} · 같은 길이의 렌더링 부하를 유지합니다.`,
        imageUrls: [],
      }));
      const metrics = {
        topicRenders: 0,
      ...
      samples.push({
        ...
        topicsAtFirstCommit: metrics.topicsAtFirstCommit,
        topicsAt250Ms,
        uniqueTopicsAfterScroll: metrics.mountedIds.size,
        topicRenders: metrics.topicRenders,
      ...
      const results = SIZES.flatMap((size) =>
        ['before', 'after'].map((variant) => {
          ...
          return {
            ...
            topicsAtFirstCommitMedian: get('topicsAtFirstCommit', 0.5),
            topicsAt250MsMedian: get('topicsAt250Ms', 0.5),

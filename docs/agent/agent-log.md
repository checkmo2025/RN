...
# 2026-07-07 14:48:31 KST 하단 탭 재클릭 시 책 검색 닫힘 처리
...
# 2026-07-08 19:04:15 KST 기능명세서 최신화
...
# 2026-07-08 20:34:59 KST iOS 1.1.7 배포 버전 준비
...
# 2026-07-09 15:04:11 KST 앱 온보딩 캐러셀 추가

- 제공된 온보딩 PNG 5장을 `assets/onboarding`에 추가
- 첫 실행 1회 노출되는 온보딩 캐러셀과 `SecureStore` 기반 본 여부 저장 로직 추가
- 기존 placeholder 온보딩 코드를 PNG 슬라이드 기반 화면으로 교체하고 앱 버전 게이트와 겹치지 않게 조정

# 2026-07-09 15:13:26 KST 온보딩 이미지 품질 및 페이지 표시 개선
...
# 2026-07-09 15:34:57 KST TODO 실기기 QA 통과 정리

- Play Store 제출 준비, Android 실기기 QA, 뒤로가기 액션 확인 항목을 사용자 통과 기준으로 완료 처리
- 배포 앱 채팅 웹소켓, 로그인 세션 유지, 한줄평 별점 드래그 입력 QA 항목을 완료 처리

# 2026-07-10 12:10:23 KST 1.1.8 심사 준비 체크포인트

- 1.1.8 심사 준비 상태에서 워킹트리가 깨끗한지 확인
- RN 하드코딩 약관 문구 정렬 전 체크포인트 로그 기록

# 2026-07-10 12:15:59 KST RN 하드코딩 약관 문구 동기화

- RN `termsDocuments` 4개 문서를 웹 support v1 약관 문구 기준으로 갱신
- 개인정보 문서에 채팅, 신고/차단, 푸시 토큰, 설치 식별자, 알림 설정 항목 반영
- 이용약관에 커뮤니티 콘텐츠, 채팅, 신고/차단 운영 문구와 2026년 7월 9일 시행일 반영

# 2026-07-10 12:59:22 KST 책이야기 상세 본문 로드 안정화

- 상세 API 응답 전 목록 미리보기 본문이 상세 원문처럼 노출되지 않도록 로딩 상태 분리
- 상세 로드 실패 시 오류 안내와 재시도 버튼을 표시하고 인증·권한 오류 문구 보강
- 오래된 상세 요청 응답 적용을 차단하고 개발 로그에 목록·상세 본문 길이만 기록
- `npm run check`로 typography/spacing/typecheck/expo-doctor 검증

# 2026-07-10 13:36:26 KST 상세 API 독립 로드 및 실패 상태 분리

- 소식 상세 실패·빈 응답 시 목록 요약문을 본문으로 대체하지 않고 오류 안내와 재시도 표시
- 공지 본문과 댓글을 동시에 독립 요청해 성공 결과를 각각 반영하고 댓글 영역 재시도 추가
- 책장 기본 정보와 활성 탭만 우선 로드하며 발제·한줄평·정기모임의 로딩·성공·실패 상태 및 지연 로드 분리
- 빈 결과는 API 성공 시에만 표시하고 섹션별 실패 UI에서 개별 재시도 제공

# 2026-07-10 13:38:49 KST 프로필 소개 전체 표시

- 내 프로필 2줄, 타인 프로필 3줄 표시 제한을 제거해 저장된 소개 40자를 전부 표시
- 프로필 소개 입력·저장 최대 40자 제한과 모임 상세 설명 표시는 기존대로 유지

# 2026-07-10 14:12:15 KST 푸시 알림 재등록 및 기기 권한 안정화

- 푸시 OFF·로그아웃 후에도 기존 `installationId`를 보존해 백엔드에서 동일 기기를 재활성화하도록 조정
- OS 알림 권한과 Android 알림 채널 상태를 스위치에 반영하고 기기 설정 복귀 시 자동 재등록 추가
- 동시 등록·해제 경쟁을 방지하고 `npm run check`·대상 ESLint·diff 검사 통과

# 2026-07-10 14:25:15 KST 한글 닉네임 지원 계획 작성

- 한글·호환 자모 허용, 모든 공백 금지, 최대 20자 및 NFC 정규화 정책 확정
- BE·웹 FE·RN의 수정 위치, 테스트 행렬, 배포 순서와 완료 조건을 계획 문서로 정리

# 2026-07-10 14:56:33 KST 모임 온보딩 및 사용법 다시보기 추가

- 모임 홈·공지·책장·발제·한줄평·정기모임·관리 온보딩 에셋 7장과 캐러셀 추가
...
# 2026-07-10 15:06:05 KST 버전 정보 이력 표시
...
# 2026-07-11 13:58:13 KST 앱 인증 세션 회전 및 경합 안정화

- SecureStore Refresh Token 저장·교체·삭제를 직렬화하고 세대 검증으로 늦은 응답이 새 로그인 세션을 덮어쓰지 않도록 보강
- 앱 시작·foreground·API 401에서 토큰을 회전하고 일시 실패에는 세션을 보존하며 확정 무효 응답만 로그아웃 처리
- 앱 로그인·OAuth·로그아웃을 BE `X-Refresh-Token` 계약에 맞추고 인증 흐름 검증 완료

# 2026-07-11 14:24:36 KST 책 검색·모임 관리 UX 개선

- 책 이야기·책장 생성 도서 검색에 무한 스크롤, 요청 경합 방지 및 누적 캐시 갱신 적용
- 모임 상세 공개 여부 문구를 정리하고 회원 역할에서 개설자 위임을 개설자 전용으로 제한
- 역할 변경 메뉴의 전역 비활성 opacity·중복 눌림 피드백을 제거해 깜빡임 개선
- iOS E2E는 만료된 테스트 세션으로 기능 진입이 막혀 결과를 기록

# 2026-07-11 14:27:55 KST 마이페이지 입력·임시저장 복귀 개선

- 자기소개를 단일 줄 입력으로 전환하고 붙여넣기 줄바꿈을 공백으로 정규화
- 임시저장 책 이야기 편집을 닫을 때 마이페이지의 내 책 이야기 탭으로 복귀하도록 보강

# 2026-07-11 15:35:32 KST iOS 로컬 배포 설정 동기화

- iOS 로컬/Xcode 빌드 번호를 39로 동기화하고 배포용 앱 메타데이터를 정리
- 다음 Archive·Upload 절차를 위한 로컬 릴리스 런북과 사전 확인 규칙을 추가

# 2026-07-11 15:57:10 KST 서재·모임 문의 문구 개선

- 마이페이지 도서 찜 해제 토스트를 `내 서재에서 제거했습니다.`로 변경
- 모임 홈 문의 모달에서 URL을 숨기고 등록된 링크 이름만 표시하며 링크 이동 동작을 유지

# 2026-07-11 16:15:54 KST 가입 신청 관리 프로필 이동 개선

- 가입 메시지 팝업에 명시적인 `X` 닫기 버튼을 추가
- 프로필 보기 시 관리 모달만 임시로 숨기고 뒤로가기하면 가입 신청 관리 화면을 복원

# 2026-07-11 16:25:02 KST 가입 신청·로그아웃 토스트 개선

- 모임 가입 신청 입력창 높이를 줄이고 모달 내부 토스트 호스트로 글자 수 초과 안내가 최상단에 표시되도록 개선
- 수동 로그아웃 정리 요청의 인증 만료 알림을 억제하고 실제 세션 만료 문구를 별도로 통일

# 2026-07-11 16:34:41 KST 모임 검색·가입 신청 상태 개선

- 가입 신청 비활성 버튼의 부모 눌림 투명도를 차단하고 모달을 닫을 때 작성 중인 신청 사유를 즉시 폐기
- 검색어와 필터가 기본값으로 복원되면 섹션 헤더도 `독서 모임 추천`으로 돌아오도록 수정

# 2026-07-11 16:36:16 KST 네이티브 모달 토스트 레이어 보강

- 헤더 검색·알림, 책이야기 책 검색, 조 관리, 앱 업데이트 모달에 표시 중 전용 토스트 호스트 추가
- 공용 `DialogOverlay`에 토스트 호스트를 연결해 발제·한줄평과 모임 문의하기 안내가 모달 최상단에 표시되도록 개선

# 2026-07-11 16:58:36 KST 가입 신청 모달 키보드 UX 개선

- 가입 신청 입력칸 높이를 복원하고 모달의 최소·최대 높이를 확장해 글자 수 카운터가 처음부터 보이도록 조정
- 입력창 포커스와 키보드 표시 완료 시 모달 내부를 하단으로 이동해 카운터와 버튼 노출 유지

# 2026-07-11 17:15:41 KST 모임 책장·회원 관리 상호작용 개선

- 한줄평 별점 변경 시 선택 햅틱을 제공하고 동일 값의 반복 햅틱을 방지
- 회원 관리의 `회원 탈퇴` 문구를 `탈퇴 처리`로 변경
- 가입 처리 팝업에 닫기 버튼을 추가하고 삭제 표현을 가입 거절로 통일

# 2026-07-11 17:36:55 KST 공지·모임 관리 상호작용 개선

- 한줄평 변경 후 책장 평균 별점과 공지 연결 책장 별점을 최신 상태로 동기화
- 공지 작성 화면의 키보드 닫기 동작과 iOS 투표 시간 다이얼의 시·분 연속 선택을 개선
- 모임 관리 바텀시트를 전체 너비와 공용 닫기 애니메이션 방식으로 통일

# 2026-07-11 17:54:11 KST 공지 투표 입력 UI 개선

- iOS 시간 선택기를 공용 바텀시트의 배경·전환 효과와 전체 너비 중앙 정렬로 통일
- 시간 선택기의 취소·완료 버튼 크기를 맞추고 시작·종료 시간 라벨을 추가
- 익명·실명을 라디오형 택 1 세그먼트로 묶고 중복 가능 옵션을 별도로 구분

# 2026-07-11 18:43:36 KST 프로필·모임 관리·공지 투표 UX 보강

- 마이페이지와 다른 회원 프로필 사진의 전체 화면 확대를 추가하고 다른 회원의 공개 모임 카드·방문 메뉴 이동을 인증 흐름과 연결
- 가입 처리·회원 역할 팝업 문구와 닫기 버튼을 정리하고 관리 하위 화면 뒤로가기 시 관리 바텀시트로 복귀하도록 개선
- 공지 투표 시간 선택 레이아웃을 안정화하고 투표자 사진·프로필 이동·본인 클릭 안내 및 투표 수정 불가 문구를 보강
- 수동 로그아웃 구간의 인증 만료 알림과 늦은 401을 억제해 로그아웃 완료 안내만 표시하도록 수정

# 2026-07-11 18:54:34 KST 앱 버전 1.2.0 심사 준비
...
# 2026-07-12 01:21:46 KST 운영진 소개서 레이아웃 후속 조정
...
# 2026-07-13 13:27:01 KST UTM 홍보 링크 운영 문서 추가

- 카카오톡 5개 방과 인스타그램·커뮤니티·메시지·이메일·QR용 UTM 링크 예시를 정리
- 채널·캠페인·홍보 위치 명명 규칙과 GA4 트래픽 획득 확인 절차를 문서화

# 2026-07-13 14:32:03 KST 모바일 채팅·태블릿 책이야기·iOS Firebase 개선

- Android 채팅 키보드 표시 시 입력창이 가려지지 않도록 화면 높이 조정을 적용
- 태블릿 책이야기를 화면 너비에 따라 최대 2열로 배치하고 큰 휴대폰은 기존 1열을 유지
- iOS Firebase 정적 프레임워크 연결과 Xcode 26 빌드 호환 설정을 보강
- 전체 품질 검사와 Firebase 포함 iOS arm64 빌드 통과

# 2026-07-13 14:38:18 KST 태블릿 홈·모임 레이아웃과 iPad 가로 화면 개선

- 홈 화면 책이야기를 태블릿에서 2열로 배치하고 큰 휴대폰은 1열을 유지
- iPad에서 세로·가로 전체 화면을 지원하도록 방향 설정을 분리
- 모임 홈의 회원 카드를 태블릿 화면 너비에 맞는 1열 전체 폭으로 조정

# 2026-07-13 14:45:09 KST 댓글 버튼·정기모임 구성원 팝업 UX 개선

- 책이야기 댓글·대댓글·수정 등록 버튼의 활성·비활성 색을 공지사항과 통일
- 정기모임 조 구성원 목록을 바깥 화면 터치와 Android 뒤로가기로 닫히도록 개선

# 2026-07-13 15:35:51 KST 태블릿 책이야기 카드 푸터 정렬 개선
...
# 2026-07-14 19:08:27 KST 운영진 소개서 슬라이드 내용·아이콘 정리

- 표지 컬러·문구·스토어 로고와 도메인을 정리하고 커버 폰을 정면 아이폰(홈 화면)으로 교체
- 목차 슬라이드를 삭제하고 전 슬라이드 헤더 페이지 번호를 02~09로 재정렬
- 도구 분산 슬라이드를 "여러 도구에 둘러싸여 힘들어하는 운영진" 구도로 재구성(카카오톡·노션·책·모임·질문 아이콘 + 정장 아바타), 화면 예시 폰에 모임 관리·정기모임 스크린샷 삽입
- 여러 슬라이드 제목·하단 문구를 정리하고 한 달 시나리오 슬라이드를 삭제

# 2026-07-14 20:05:38 KST 운영진 소개서 PDF 산출

- 최신 운영진 소개서 HTML을 10페이지 16:9 가로형 PDF로 생성
- 전체 페이지 렌더링과 주요 텍스트·링크 유지 여부를 검증
- 별도 PDF 복사본을 데스크톱 최상위 폴더로 이동

# 2026-07-20 18:30:04 KST 운영진 소개서 시각 요소·스토어 안내 개선

- 마지막 페이지를 표지와 같은 브라운 톤으로 정리하고 App Store·Google Play QR 및 링크를 추가
- 7페이지 기능 카드를 확대하고 하단 설명 바를 제거한 뒤 지정 이미지와 새 제목으로 교체
- 앱 기능 화면 사이에 단계별 화살표를 추가하고 단계 제목 가독성을 개선
- 중간 페이지 로고 테두리를 제거하고 마지막 페이지 하단을 표지와 동일한 스토어·도메인 구성으로 통일

# 2026-07-20 18:36:05 KST 운영진 소개서 본문 가독성 개선

- 4페이지 기능 설명 글씨를 확대하고 행 안의 줄바꿈을 점검
- 6페이지 비교표 머리글·행 제목·비교 내용 글씨를 확대해 한 줄 정렬을 유지
- 10페이지 책모 로고를 확대하고 크기에 맞춰 모서리 곡률을 조정

# 2026-07-20 19:52:05 KST 운영진 소개서 모바일 가로보기 개선

- 휴대폰 세로 화면에 가로 회전 안내를 추가하고 모바일 주소창·안전 영역 높이를 반영
- 가로 화면에서 슬라이드가 오른쪽 아래로 밀리지 않도록 화면 중앙 기준 축소 배치를 적용

# 2026-07-20 19:56:11 KST 책이야기 댓글 삭제 확인창 추가
...
# 2026-07-21 18:59:51 KST 운영진 소개서 Safari 중앙 배치 개선

- Safari에서 슬라이드가 오른쪽 아래로 밀리지 않도록 실제 컨테이너 크기 기반 확대율·여백 계산을 적용
- 퍼센트 이동을 제거하고 픽셀 좌표로 중앙 배치하며 회전·주소창 높이 변경 시 재계산하도록 보강

# 2026-07-21 21:42:17 KST 앱 버전 1.2.4 상향
...
# 2026-08-08 23:55:27 KST RN 정기모임 조 진입 단계 제거

- 정기모임 탭에서 조 미리보기와 화살표 단계를 제거하고 선택한 조의 발제·참여자 기능을 바로 노출
- 일반 회원의 본인 조 기본 선택과 조별 발제 실시간 연결·채팅 노출 조건을 새 화면 구조에 맞게 정리
- TypeScript, ESLint, 타이포그래피·간격 검사를 완료

# 2026-08-09 00:13:34 KST RN 책장 작성 버튼 통일

- 책장 발제·한줄평 탭의 `+` 버튼을 정기모임과 동일한 연필 아이콘·텍스트 버튼으로 변경
- 기존 작성 모달 동작과 접근성 라벨을 유지하고 TypeScript·ESLint·타이포그래피·간격 검사를 완료

# 2026-08-09 00:54:08 KST RN 한글·대소문자 닉네임 정책 적용

- 회원가입·프로필 변경에서 한글과 영문 대소문자를 허용하고 공백·백틱·물결표·이모지 등 정책 밖 문자는 오류로 안내
- NFC 정규화와 대소문자 비구분 비교를 공통화해 본인의 대소문자 표기 변경은 자동 통과하도록 적용
- 한글 닉네임의 프로필·팔로우·서재·책 이야기 경로와 딥링크를 안전하게 인코딩·해석하도록 보강
- TypeScript·ESLint·타이포그래피·간격 검사와 닉네임·딥링크 정책 확인을 완료

# 2026-08-09 18:13:20 KST 책이야기 자동 게시 기반 구축

- macOS 키체인 기반 로그인·토큰 회전과 세션 점검 도구를 추가
- 알라딘 도서 확인, 초안 분량·금칙어·이모지 검증, 중복 방지 및 게시 후 상태 확인을 자동화
- 게시 성공 후에만 로컬 Markdown 목록을 완료 처리하고 목록·초안은 Git 추적에서 제외
- TypeScript·Node 문법 검사와 실계정 세션·미리보기·게시 검증을 완료

# 2026-08-09 23:20:50 KST 책이야기 계정·요일별 자동 게시 분리

- 관리자와 감성회원 계정의 갱신 토큰을 macOS 키체인에 분리하고 글 컨셉에 맞는 계정만 사용하도록 변경
...
# 2026-08-20 16:08:16 KST RN 책이야기·댓글 이미지 첨부

- 최대 5장 다중 선택·삭제·순서 변경·재시도를 지원하는 공통 첨부 계층과 전체화면 갤러리를 추가
- 책이야기 작성·수정·임시저장과 댓글·대댓글 작성·수정·상세 조회에 이미지 흐름을 연결
- 공지 댓글 작성·수정·조회에도 공통 첨부 UX를 적용하고 전환·취소·삭제 시 상태를 정리
- API 이미지 계약과 iOS 사진 권한 문구를 동기화하고 타입·린트·타이포그래피·간격·설정 검사를 완료

# 2026-08-21 15:39:10 KST RN 이미지 첨부 버튼 입력창 통합

- 댓글·대댓글·댓글 수정 입력창 오른쪽에 44×44 이미지 첨부 아이콘을 반응형으로 배치
- 책이야기 본문의 사진 첨부 제목·텍스트 버튼을 제거하고 동일한 입력창 내부 아이콘으로 통일
- 선택 후 이미지 개수·썸네일·삭제·순서 변경 UX를 유지하고 타입·린트·타이포그래피·간격 검사를 완료

# 2026-08-21 16:08:07 KST RN 책이야기 사진 첨부 행 정돈

- 본문 글자 수 아래에 구분선을 두고 사진 아이콘은 왼쪽, 이미지 개수는 오른쪽에 같은 행으로 배치
- 사진 선택 후 썸네일·삭제·순서 변경 흐름을 유지하고 하단 버튼 문구를 `등록·임시저장`으로 정리
- 타입·린트·타이포그래피·간격 검사를 완료

# 2026-08-21 16:11:09 KST RN 구분 문구 표기 통일

- 프로필·설정·로그인·신고 UI의 슬래시 구분 문구를 가운데점 또는 자연스러운 공백 표현으로 통일
- 장르 카테고리와 약관 서술 문구는 기존 의미를 유지
- 타입·린트·타이포그래피·간격 검사를 완료

# 2026-08-21 16:28:33 KST 책이야기 목록·금칙어 검증 보완

- 자동화 목록의 `예정 일시 (KST)` 열을 읽고 게시 결과 열을 정확히 갱신하도록 목록 파서를 보완
- 정상 단어의 `발`을 욕설로 오인하지 않도록 금칙어 정규식을 수정
- 관리자·감성 다음 일정 파싱, 금칙어 오탐 방지 및 Node 문법 검사를 완료

# 2026-08-27 23:49:23 KST RN 소식 캐러셀 조회 공통화 및 Ponytail 스킬 추가

- 홈·소식 화면의 전체 페이지 수집·중복 제거·프로모션 필터를 공통 API 함수로 통합하고 기존 기본 콘텐츠·오류 처리·상세 이동을 유지
- 페이지 수집, 빈 결과, 반복 커서, 오류 전달, 새로고침을 확인하는 회귀 테스트 5개를 추가
- 프로젝트 전용 Ponytail 스킬과 원본 MIT 라이선스를 추가하고 전역 설정·기존 AGENTS.md는 유지
- 회귀 테스트·TypeScript·변경 파일 ESLint·스킬 형식 검증을 완료했으며 시뮬레이터 검증은 미실행

# 2026-08-28 19:34:00 KST Ponytail 리팩터링 기록 템플릿화

- 기능별 제목·파일명, 변경 전후 코드, 가설·근거·측정·결론을 포함하는 포트폴리오용 기록 템플릿과 목록을 추가
- 핵심 구현 200줄 기준과 동작 보존 계약, 정적 지표 및 조건부 런타임 성능 측정 원칙을 문서화
- 소식 캐러셀 공통화 기록을 시범 문서로 재구성하고 프로젝트 AGENTS 규칙에 향후 기록 절차를 연결
- 문서 링크·코드 블록·제목·필수 섹션과 변경 수치를 검증했으며 앱 코드는 변경하지 않음

# 2026-08-28 19:41:14 KST 마이페이지 내 소식 페이지 수집 공통화

- `내 소식`의 직접 커서 수집 루프를 기존 공통 유틸 호출로 바꾸고 로그인·매핑·오류·토스트·로딩 및 책 이야기 동작을 유지
...
# 2026-09-06 18:47:06 KST 모임 검색 결과 목록 가상화

- 모임 검색·추천 결과를 `FlatList`로 가상화하고 데이터·순서·필터·가입 신청·방문·오류·권한 동작을 유지
- iOS Release에서 조건별 30회 측정해 19개 35.5%, 100개 81.7%, 500개 97.0% 초기 커밋 단축과 7개 9.6% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS export·Release 빌드와 로그인 시뮬레이터 화면을 검증

# 2026-09-06 19:16:42 KST 마이페이지 알림 목록 가상화

- `내 알림` 목록을 `FlatList`로 가상화하고 데이터·순서·문구·읽음 처리·이동·오류·새로고침 동작을 유지
- iOS Release에서 조건별 30회 측정해 19개 13.5%, 100개 81.5%, 500개 93.5% 초기 커밋 단축과 7개 25.3% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS Release 빌드와 로그인 시뮬레이터 화면을 검증

# 2026-09-06 22:16:23 KST 헤더 도서 검색 결과 목록 가상화

- 전역 헤더의 도서 검색 결과를 `FlatList`로 가상화하고 데이터·순서·문구·좋아요·작성·상세 이동과 하단 240px 추가 조회 조건을 유지
- iOS Release/Hermes에서 조건별 30회 측정해 19개 32.1%, 100개 81.7%, 500개 96.7% 초기 커밋 단축과 7개 6.9% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS Release 빌드와 `book_cafe` 로그인 시뮬레이터 검색·상세·복귀를 검증

# 2026-09-07 11:54:35 KST 책이야기 도서 선택 검색 결과 가상화

- 책이야기 작성의 도서 선택 검색 결과를 `FlatList`로 가상화하고 데이터·순서·문구·선택·추가 조회 동작을 유지
- iOS Release/Hermes에서 조건별 30회 측정해 19개 23.1%, 100개 78.3%, 500개 94.6% 초기 커밋 단축과 7개 3.3% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS Release 빌드와 `book_cafe` 로그인 시뮬레이터 검색·선택·복귀를 검증

# 2026-09-07 16:42:58 KST 모임 책장 도서 검색 결과 가상화

- 모임 책장 생성·수정의 도서 검색 결과를 `FlatList`로 바꾸고 데이터·순서·문구·선택 상태·추가 조회 동작을 유지
- iOS Release/Hermes에서 조건별 30회 측정해 19개 23.5%, 100개 77.4%, 500개 96.2% 초기 커밋 단축과 7개 22.5% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS Release 빌드와 `book_cafe` 로그인 시뮬레이터 검색·선택·복귀를 검증

# 2026-09-07 17:03:52 KST 마이페이지 내 책 이야기 목록 가상화
...
# 2026-09-08 22:58:25 KST 다른 사용자 프로필 책 이야기 목록 가상화

- 다른 사용자 프로필의 `책 이야기` 2열 카드를 `FlatList`의 행으로 가상화하고 데이터·순서·문구·상세 이동·로딩 동작을 유지
- iOS Release/Hermes에서 조건별 30회 측정해 100개 69.2%, 500개 94.8% 초기 커밋 단축과 실제 규모에 가까운 19개 6.4% 증가를 기록
- TypeScript, 대상 ESLint, 디자인 토큰 검사, iOS Release 빌드와 `book_cafe` 로그인 상태의 `책읽는버즈` 프로필·상세·탭 전환을 검증

# 2026-09-12 16:03:24 KST 다른 사용자 프로필 공개 서재 가상화

- 공개 서재의 3열 책 카드를 기존 바깥 `FlatList`의 행으로 가상화하고 데이터·순서·책 상세 이동과 새로고침 중 기존 책·스켈레톤 표시를 유지
- 핵심 변경 84줄, 0~500권·상세 이동·로딩·빈 상태·탭 분리 회귀 검사 6개 통과; TypeScript·대상 ESLint·디자인 토큰·iOS 프로덕션 export 검증
- 변경 전후 코드·가설·구조 지표를 최적화 문서에 기록하고 런타임 성능은 미측정으로 명시

# 2026-09-12 16:23:35 KST 공개 서재 가상화 Release 성능 실험

- 실제 변경 전후 화면 코드를 추출해 iOS 18.6 Release/Hermes에서 조건별 예열 5회·본 측정 30회 수행하고 측정기·재현 절차·원본 JSON 보관
- 100·500권 초기 커밋 중앙값 48.4%·88.7% 감소, 첫 커밋 카드 30권으로 제한; 7·19권은 35.4%·8.1% 증가해 부분 확인으로 기록
- API·원격 이미지·실기기 성능은 범위 밖으로 명시하고 제품 진입점 재빌드·정상 앱 재설치 완료, TypeScript·공백 검사 통과

# 2026-09-12 16:29:47 KST 사용자 프로필 책 이야기 페이지 수집 공통화

- 프로필 책 이야기 수집을 기존 공통 유틸로 대체해 수집 코드 16→4줄로 축소하고 최대 100페이지·ID 중복·반복 커서·오류·순서 보존
- 실제 전후 로더에 115개 합성 응답 사례를 제공해 API·결과·상태·오류 불일치 0건, 책 이야기 요청 각 518회 확인; 원본 결과·재현 검사·최적화 기록 보관
- 페이지 수집 8개·렌더 회귀 6개 검사, TypeScript·대상 ESLint·디자인 토큰·iOS 프로덕션 export 통과; 응답속도 개선은 주장하지 않음

# 2026-09-12 17:04:35 KST 모임 관리 멤버 목록 가상화

- 모임 관리의 멤버 카드 목록을 `FlatList`로 가상화하고 데이터·순서·문구·새로고침·역할 수정 동작을 유지
- iOS Release/Hermes에서 조건별 30회 측정해 500명 첫 커밋 카드 500→10개, 초기 커밋 중앙값 184.93→6.56ms를 확인
- 회귀 검사 5개, TypeScript·대상 ESLint·디자인 토큰·일반 iOS Release 빌드 통과 후 실험 앱을 일반 앱으로 복원하고 시뮬레이터 종료

# 2026-09-12 17:50:48 KST 모임 공지 상세 댓글 가상화

- 공지 상세의 루트 세로 목록을 `FlatList`로 전환하고 댓글 ID·순서·문구·권한·작성·수정·추가 페이지 호출 동작을 유지
- iOS Release/Hermes 조건별 30회에서 댓글 500개 초기 커밋 중앙값 220.19→8.44ms, 첫 커밋 댓글 500→10개 확인; 동일 거리 스크롤 FPS는 변화 없어 부분 확인
- 회귀 검사 6개, TypeScript·대상 ESLint·디자인 토큰·일반 iOS Release 빌드 통과; 원본 JSON·재현 측정기·최적화 기록 보관

# 2026-09-12 18:38:39 KST 마이페이지 신고 내역 목록 가상화

- 신고 내역을 전용 `FlatList`로 가상화하고 ID·순서·카드 문구·대상 이동·로딩·빈 결과·오류·로그인 동작을 유지
- iOS Release/Hermes 조건별 30회에서 500개 초기 커밋 중앙값 160.49→6.50ms, 첫 커밋 카드 500→10개 확인; 7개는 느려지고 스크롤 FPS 개선은 없어 부분 확인
- 회귀 검사 5개, TypeScript·대상 ESLint·디자인 토큰·벤치마크 및 일반 iOS Release 빌드 통과; 원본 JSON·재현 측정기·최적화 기록 보관

# 2026-09-12 19:09:55 KST 홈 오프라인 반복 로딩 및 기본 소식 안내 수정

- 홈 책이야기 조회 실패 후 자동 추가 요청과 중복 로딩을 차단하고 기존 게시물 유지·오류 안내·다시 시도 버튼을 적용
- 홈·소식 탭의 기본 예시 배너 3개를 `책모에 올라갈 소식을 문의하세요!`와 `checkmo2025@gmail.com` 안내 1개로 교체하고 영어 번역 반영
- 홈·소식 회귀 검사 14개, TypeScript·대상 ESLint 통과; 모의 실패 후 끝 도달 이벤트 20회에서 요청 21→1회 확인, 실제 기기 프레임 성능은 미측정

# 2026-09-13 13:56:11 KST iOS 1.2.8 빌드 45 제출 준비

- 사용자 확인 버전 1.2.8·빌드 45를 Expo·패키지·iOS 설정과 로컬 Expo 런타임에 동기화하고 외부 의존성 버전은 유지

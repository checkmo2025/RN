...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **편집 중 이탈 경고 통일 QA** | 공지 작성/수정, 발제/한줄평 작성/수정, 모임 정보 수정, 책장 생성/수정, 책이야기 글/댓글/대댓글, 마이페이지 이메일/비밀번호 변경, 회원가입 프로필 단계 | 변경 없이 닫기 → 경고 없음. 변경 후 헤더 닫기/뒤로가기/탭 전환/Android 하드웨어 백 → 저장 안 됨 경고. 입력 후 원복 → 경고 없음. 저장/등록/수정 완료 후에는 경고 없이 정상 이동. | 2026-06-14 | 2026-06-14 |
...
| ⬜ | **공지 제목 입력 제한/토스트** | 모임 → 관리 → 공지 작성 | 수정 후 재확인. 제목 입력칸이 긴 제목을 아래로 길게 보여주는지, 50자 초과 입력/붙여넣기 시 토스트 노출 + 추가 입력 차단되는지, 카운터가 입력칸 위 라벨 우측에 표시되는지 확인. | 2026-06-14 | 2026-06-14 |

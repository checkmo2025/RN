...
| ------ | ------ | ------ | ------ | ------ |
| ⬜ | **앱 버전 정책 조회 재활성화** | BE 앱 버전 정책 API가 아직 머지되지 않아 RN 부팅 시 `/app/version?platform=ios|android` 조회를 `useAppVersionGate`에서 임시 주석 처리했다. BE 머지 및 운영 배포 후 조회/버전 비교 로직을 복구하고 강제 업데이트/권장 업데이트 모달을 실기기에서 확인한다. DoD: BE 머지된 환경에서 iOS/Android 정책 조회 200, 최신 버전 통과, 최소 지원 버전 미만 강제 업데이트 모달, 최신 버전 미만 권장 업데이트 모달 확인. | 2026-06-29 | 2026-06-29 |

...
| ⬜ | **한줄평 별점 드래그 입력** | 모임 → 책장 → 한줄평 추가/수정 → 평점 | 별점 영역을 누르거나 좌우로 밀면 배민/요기요 평점처럼 0.5점 단위로 자연스럽게 변경됨. 세로 스크롤을 방해하지 않고, 표시되는 별/점수 텍스트/등록 payload가 같은 값으로 유지됨. | 2026-07-06 | 2026-07-06 |
...
| ⬜ | **선택형 칩/버튼 활성 배경 대비** | Android 중심 → 책장 기수/정기모임 칩, 모임 생성·수정 카테고리/대상, 공지 투표/작성 옵션, 책 검색/책장 책 선택, 마이페이지 언어·신고·팔로우, 책이야기 구독 | active/selected 배경을 `subbrown4`보다 진한 `subbrown3`로 올린 뒤, 선택 전후 배경색 변화가 Android에서도 흰 배경과 명확히 구분되는지 확인. 탭 밑줄형 UI는 변경 대상이 아니며 기존처럼 밑줄/텍스트 강조만 보여야 함. | 2026-07-06 | 2026-07-06 |

...
| ⬜ | **책이야기 임시저장 API 연결** | 책이야기 저장 플로우에서 임시저장을 `status: DRAFT`로 연동. 기준 경로: `POST /api/book-stories`, `PATCH /api/book-stories/{bookStoryId}`. 완료 기준(DoD): 임시저장 생성/수정, 재진입 시 DRAFT 이어쓰기, 발행 시 `PUBLISHED` 전환까지 확인. | 2026-05-20 | - |
...
| ⬜ | **공동** | `MYPAGE-02` | 기본 프로필 이미지 서버 저장 불가 | 기본 프로필 선택 시 RN이 `imgUrl: ''` 전송 → BE가 null 저장 → 다른 사람이 조회할 때 색상 미반영, 고정 SVG만 표시. RN에 번들된 에셋은 로컬 경로라 다른 기기에서 접근 불가. **해결 방향**: BE에서 기본 프로필 SVG를 CDN에 호스팅 후 공개 URL 10개(색상별) 제공 → RN이 선택 색상에 맞는 URL을 imgUrl로 저장. 스키마 변경 불필요한 방식. 완료 기준(DoD): 기본 프로필 색상 저장 후 타인 조회 시 동일 색상 표시. | 2026-05-08 | 2026-05-08 |

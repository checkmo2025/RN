...
| 🔄 | **공지 작성/수정 입력·투표 QA** | 모임 → 관리 → 공지 작성/수정 | 코드 수정 완료(2026-06-14), 실기기 육안 잔여. ①투표 날짜를 오프셋 없는 `LocalDateTime` 포맷으로 전송해 투표 있는 공지 등록/수정이 되는지 ②투표 기간이 기기 네이티브 datepicker(iOS 스피너/Android 다이얼로그)로 뜨는지(※iOS는 `npx expo run:ios` 재빌드 필요) ③투표 항목 최대 6개(6개면 "항목 추가" 숨김) ④"투표" 옆 빨간 문구 "투표가 있는 공지사항은 수정이 불가합니다" 노출 ⑤제목/내용 입력 시 입력칸 아래 진동 없음 ⑥제목 50자/내용 2000자 제한·토스트, 투표 항목 3번부터 X 삭제 확인. | 2026-06-14 | 2026-06-14 |
...
| ⬜ | **책이야기 글 작성/수정 화면 꺼짐** | 책이야기 상세 → 내 글 메뉴 → 수정하기 / 글 작성·수정 저장 | 수정 후 재확인. 수정 화면 진입 시 멀티라인 입력 자동 포커스 제거 후 앱이 꺼지지 않는지, 본문 탭 입력과 수정 저장이 정상인지 확인. | 2026-06-14 | 2026-06-14 |
...
| 🔄 | **모임 정보 수정 사진/책장 버튼 상태** | 모임 → 운영진 관리 → 모임 수정 / 책장 생성·수정 | 모임 정보 수정 사진 UI는 실기기 통과 확인. 책장 `meetingTime`이 `+09:00` 오프셋으로 전송돼 400 나던 문제를 오프셋 없는 `LocalDateTime` 포맷으로 수정 완료(2026-06-14) — **모임 날짜를 넣고** 책장 생성/수정 시 등록/저장이 실제로 성공하는지 실기기 재확인 필요. | 2026-06-14 | 2026-06-14 |

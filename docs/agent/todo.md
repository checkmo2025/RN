...
| ⬜ | **책이야기 댓글 등록 버튼 문구** | 책이야기 상세 → 댓글/대댓글 입력 영역 | 일반 댓글과 대댓글 작성 버튼이 `입력`이 아니라 `등록`으로 표시됨. 댓글 수정 상태는 기존처럼 `수정`으로 표시됨. | 2026-07-06 | 2026-07-06 |
| ⬜ | **공지사항 댓글 등록 버튼 문구/크기** | 모임 상세 → 공지사항 → 공지 상세 → 댓글 입력 영역 | 댓글 작성 버튼이 `입력`이 아니라 `등록`으로 표시되고, 버튼 크기/간격/입력창 높이가 책이야기 댓글 입력 영역과 동일한 기준으로 보임. 댓글 수정 상태는 기존처럼 `수정`으로 표시됨. | 2026-07-06 | 2026-07-06 |
...
| ⬜ | **마이페이지 비로그인 프로필 스켈레톤** | 비로그인 상태 또는 세션 복구 직후 → 마이페이지 메인 | 프로필 API가 끝나기 전이나 비로그인 상태에서 `user_id`, 기본 소개글, `프로필을 불러오는 중...` 텍스트가 노출되지 않고 프로필 사진/닉네임/구독수/소개글 자리 전체가 스켈레톤으로 보임. 로그인된 상태에서는 프로필 로딩 완료 후 실제 닉네임/소개글/카테고리로 자연스럽게 전환됨. | 2026-07-06 | 2026-07-06 |
| ⬜ | **마이페이지 내 책이야기 좋아요 하트 표시** | 마이페이지 → 내 책 이야기 탭 | API 응답에서 `likedByMe`/`liked`가 true인 내 책이야기 카드는 하트가 채워진 빨간 아이콘으로 보이고, false인 카드는 빈 하트로 보임. 새로고침/재진입 후에도 좋아요 개수와 하트 상태가 API 응답과 일치함. | 2026-07-06 | 2026-07-06 |

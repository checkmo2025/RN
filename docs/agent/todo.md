...
| ------ | ------ | ------ | ------ | ------ |
| ⬜ | **앱 버전 정책 조회 재활성화** | BE 앱 버전 정책 API(`GET /app/version`) 연동 완료로 `useAppVersionGate`의 `APP_VERSION_POLICY_LOOKUP_ENABLED`를 `true`로 복구(2026-07-01). 남은 것: 실기기에서 강제/권장 업데이트 모달과 스토어 이동 확인 — 현재 업데이트 버튼 클릭 시 App Store로 이동하지 않는 이슈 조사 중(정책 응답 `storeUrl` 값 점검 필요, iOS 앱 링크 `https://apps.apple.com/app/id6777671102` 확인됨). DoD: iOS/Android 정책 조회 200, 최신 버전 통과, 최소 지원 버전 미만 강제 업데이트 모달, 최신 버전 미만 권장 업데이트 모달, 업데이트 버튼으로 스토어 이동 확인. | 2026-06-29 | 2026-07-01 |
...
| 상태 | 항목 | 어디서 | 뭘 확인하나 | 생성일자 | 최종 편집일자 |
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **모임 가입신청 입력칸 키보드 포커싱** | 모임 > 모임 카드 > 가입신청하기 > 신청 사유 입력칸 | 가입신청하기 클릭 시 신청 사유 입력칸이 자동 포커스되고, 제출 버튼이 기기별 키보드 위에 보이는지 확인. 사용자 확인 중 위치는 적절했으나 열림 중 끊김 현상이 있어 중복 스크롤 보정 제거 후 재확인 필요. | 2026-06-17 | 2026-06-28 |

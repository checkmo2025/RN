...
| ⬜ | **입력 후 버튼 클릭 시 키보드 닫힘/즉시 실행 전수 확인** | 책이야기 댓글·작성·책 검색, 공지 댓글·작성, 모임 가입신청·생성·정보수정·책장, 마이페이지 프로필/계정 변경, 로그인/회원가입/찾기, 공통 책 검색, 채팅/신고 | 키보드가 열린 상태에서 등록/저장/검색/중복확인/다음/완료/전송 버튼을 누르면 첫 터치에 기능이 즉시 실행되고 키보드도 내려감. 버튼 동작 없이 키보드만 닫히거나, 기능은 실행됐는데 키보드가 계속 남는 화면이 없어야 함. 채팅 전송은 이번 통일 기준에 따라 전송 후 키보드가 내려감. | 2026-07-06 | 2026-07-06 |

...
| 8 | `## 👀 버그수정 육안 확인` | 중하단 | 버그수정/기능변경 후 실기기 육안 검증 체크리스트 |
| 9 | `## 🐛 issue-fetch 미완료 항목` | 하단 | API/계약 이슈 추적 항목 |
| 10 | `## 상태 범례` | 문서 최하단 | 상태 표기 기준 |
...
| ⬜ | **책이야기 공유하기 문제** | StoryScreen 책이야기 상세/메뉴의 `공유하기` 동작을 재현하고 수정한다. 어디서: `StoryScreen` `handleShareStory` 및 상세 액션/더보기 메뉴. 무엇을: 공유 버튼 클릭 시 실제 OS 공유 시트 호출, 공유 문구/링크 payload, 선택된 글 기준 데이터가 맞는지 확인. 왜: 공유 액션 실패 또는 잘못된 콘텐츠 공유는 사용자 신뢰를 떨어뜨림. DoD: iOS/Android에서 공유 시트가 정상 열리고, 현재 책이야기 제목/본문/링크 정보가 의도대로 전달되며 실패 시 사용자 안내가 표시됨. | 2026-06-14 | 2026-06-14 |
| ⬜ | **회원가입 로직 수정** | AuthFlowScreen 회원가입 플로우에서 계정 생성/로그인/추가정보 저장 순서와 실패 보상 로직을 재검토한다. DoD: 회원가입 성공/실패/재시도 케이스에서 중복 계정, 누락 프로필, 화면 전환 이상이 없도록 정리. | 2026-06-13 | 2026-06-13 |
...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **공지 태그 색상 통일** | 모임 → 공지사항 리스트 / 공지 상세 | 리스트와 상세보기의 태그 색이 동일한지(고정=핀, 일반=분홍, 투표=파랑, 모임=주황). 태그가 여러 개인 공지도 상세에 모두 같은 색으로 표시되는지. | 2026-06-14 | 2026-06-14 |
| ⬜ | **공지 제목 입력 제한/토스트** | 모임 → 관리 → 공지 작성 | 제목 입력 시 길어지면 자동 높이 확장, 다 지우면 원래 높이 복귀. 50자 초과(붙여넣기 포함) 시 토스트 노출 + 추가 입력 차단. | 2026-06-14 | 2026-06-14 |
| ⬜ | **공지 내용 글자수 카운터** | 모임 → 관리 → 공지 작성 | 내용 입력창 아래 `현재/2000` 카운터가 실시간 갱신되는지, 2000자에서 입력이 멈추는지, 길어지면 높이 확장 후 max에서 내부 스크롤되는지. | 2026-06-14 | 2026-06-14 |
| ⬜ | **공지 책장 첨부 플로우** | 모임 → 관리 → 공지 작성 → 책장 연결 | "책장 선택"이 작성 모달 위 인라인 오버레이로 뜨는지(배경 탭 닫힘/가로 스크롤). 책장 토글 켜고 미선택 등록 시 토스트 + 선택창 자동 오픈. | 2026-06-14 | 2026-06-14 |
| ⬜ | **공지 등록/수정/삭제 후 동작** | 모임 → 공지 작성·수정·삭제 | 완료 후 공지 목록 탭 이동 + 상단 스크롤. 수정 진입 시 기존 본문/책장/투표/사진 복원. 본문 비어 보이던 항목이 진입 후 채워지는지(enrich). 삭제/등록 후 목록(2페이지 이상 포함) 최신 새로고침. | 2026-06-14 | 2026-06-14 |
| ⬜ | **상단 고정 공지 말줄임** | 모임 홈 상단 고정 공지 | 고정 공지 제목이 길 때 1줄 말줄임(…) 처리되는지. | 2026-06-14 | 2026-06-14 |
| ⬜ | **운영진 관리 바텀시트 제스처** | 모임 → 운영진 관리 메뉴 | 메뉴가 스크롤되고, 위로 끌면 확장 / 아래로 당기면 닫히는지. | 2026-06-14 | 2026-06-14 |
| ⬜ | **모임 생성 공개여부 안내 문구** | 모임 생성 → 2단계(공개여부) | 공개/비공개 카드 아래에 "공개여부는 나중에 모임 관리 화면에서 다시 변경할 수 있습니다." 안내가 보이는지, 간격이 어색하지 않은지. | 2026-06-14 | 2026-06-14 |
| ⬜ | **회원가입 기본 프로필 버튼 상태** | 회원가입 → 프로필 설정(5/6) | 진입 시 "기본 프로필 이미지" 버튼이 선택 안 된 상태(secondary)로 보이는지. 기본 프로필 선택 → 경고창 "확인" 누른 뒤에만 버튼이 선택 상태(primary)+아바타 강조로 바뀌는지. 사진 업로드하면 다시 선택 해제되는지. | 2026-06-14 | 2026-06-14 |

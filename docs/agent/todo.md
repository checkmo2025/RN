...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **편집 중 이탈 경고 통일 QA** | 공지 작성/수정, 발제/한줄평 작성/수정, 모임 정보 수정, 책장 생성/수정, 책이야기 글/댓글/대댓글, 마이페이지 이메일/비밀번호 변경, 회원가입 프로필 단계 | 통과: 마이페이지 이메일/비밀번호 변경, 회원가입 프로필 단계, 발제/한줄평 작성/수정. 재수정 후 확인 필요: 책이야기 글/댓글/대댓글 이탈 경고가 화면 닫힘 없이 뜨는지, 공지 작성/수정과 책장 생성/수정에서 변경 후 닫기 경고가 정상인지. 공통 기준: 변경 없이 닫기 → 경고 없음, 변경 후 닫기/뒤로가기/탭 전환/Android 백 → 경고, 원복 후 닫기 → 경고 없음. | 2026-06-14 | 2026-06-14 |
...
| ⬜ | **공지 제목 입력 제한/토스트 + 내용 스크롤** | 모임 → 관리 → 공지 작성/수정 | 수정 후 재확인. 제목 입력칸이 긴 제목을 아래로 길게 보여주는지, 50자 초과 입력/붙여넣기 시 토스트 노출 + 추가 입력 차단되는지, 카운터가 입력칸 위 라벨 우측에 표시되는지 확인. 내용이 길어져 입력창 최대 높이에 도달하면 입력창 내부 스크롤이 되는지, 공지 수정 저장 시 화면이 크게 깜빡이지 않는지 확인. | 2026-06-14 | 2026-06-14 |
...
| ⬜ | **책이야기 글 작성/수정 화면 꺼짐** | 책이야기 상세 → 내 글 메뉴 → 수정하기 / 글 작성·수정 저장 | 수정 후 재확인. 수정하기 진입, 수정 저장, 저장 실패/재시도에서 앱 화면이 꺼지거나 전체 로딩 화면으로 튕기지 않는지 확인. | 2026-06-14 | 2026-06-14 |
| ⬜ | **책이야기 댓글/대댓글 2뎁스 제한** | 책이야기 상세 → 댓글 메뉴 | 일반 댓글에는 대댓글 쓰기가 보이고, 대댓글에는 다시 대댓글 쓰기가 보이지 않는지 확인. 댓글/대댓글 수정·취소 시 화면이 꺼지지 않고 이탈 경고가 정상인지 확인. | 2026-06-14 | 2026-06-14 |
| ⬜ | **모임 정보 수정 사진/책장 버튼 상태** | 모임 → 운영진 관리 → 모임 수정 / 책장 생성·수정 | 모임 정보 수정 사진 UI가 모임 생성 사진 UI와 동일한지, 기본 이미지/사진 업로드 버튼 상태가 자연스러운지, 책장 생성/수정 화면 하단 버튼이 항상 보이는지 확인. | 2026-06-14 | 2026-06-14 |
| ⬜ | **책장 수정 진입 크래시** | 모임 → 책장 상세 → 책장 수정 | 책장 수정 클릭 시 앱이 꺼지지 않고 수정 화면이 열리는지, 수정 화면 첫 렌더가 생성 화면처럼 깜빡이지 않는지, 저장/삭제 버튼 상태가 정상인지 확인. | 2026-06-14 | 2026-06-14 |

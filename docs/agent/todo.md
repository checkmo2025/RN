...
| ------ | ------ | ------ | ------ | ------ |
| ✅ | **조별 채팅 기능 재도입** | 모임 책장 → 정기모임 → 조 상세에 채팅 FAB를 복원하고 권한별 조 선택, REST 최근 30개/상단 커서 페이징, STOMP 실시간 송수신·재연결 병합을 구현. 작성자 클릭은 프로필 사진·아이디·사용자 신고·바로가기 인라인 모달, 타인 메시지 박스 클릭은 메시지 신고로 분리하고 `⋮` 메뉴는 제거. REST 전송 함수는 복원하지 않고 STOMP 발행만 사용. DoD: 정적 검사 통과, iOS/Android 실시간·페이징·신고는 직접 테스트 항목으로 분리. | 2026-06-22 | 2026-06-22 |
...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **조별 채팅 재도입 실기기 QA** | iOS/Android 두 계정 → 모임 → 책장 → 정기모임 → 조 상세 → 채팅 FAB | 일반 회원은 소속 조만·운영진은 전체 조가 보이는지, 최근 30개/상단 과거 로드 위치 유지, 두 계정 실시간 송수신·중복 방지·재연결을 확인. 작성자 클릭 시 인라인 프로필 모달(사진/아이디/사용자 신고/바로가기), 타인 메시지 박스 클릭 시 메시지 신고가 열리고 채팅 전용 `⋮`가 없는지, 키보드·safe-area·하드웨어/스와이프 뒤로가기를 확인. | 2026-06-22 | 2026-06-22 |
...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **책이야기 본문 긴 입력** | 책 이야기 > 책이야기 작성/수정 > 본문 입력칸 | 긴 본문 입력 시 내부 스크롤 여백이 텍스트와 겹치지 않고, 커서/스크롤바가 자연스럽게 보이는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **신고 내용 긴 입력/초과 토스트** | 프로필/사용자/댓글 메뉴 등 > 신고하기 > 신고 내용 입력칸 | 긴 신고 내용 입력 시 스크롤 여백이 텍스트와 겹치지 않고, 글자수 초과 토스트가 키보드에 가리지 않고 보이는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **모임 정보 수정 소개글 초과 토스트** | 모임 > 모임 상세 > 관리 > 모임 정보 수정 > 모임 소개글 입력칸 | 소개글 글자수 초과 시 모달 내부 토스트가 화면 위에 정상 노출되는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **발제·한줄평 초과 토스트 밝기** | 모임 > 모임 상세 > 책장/발제·한줄평 작성 > 발제 내용 또는 한줄평 내용 입력칸 | 글자수 초과 토스트 배경이 너무 어둡지 않고, 키보드 위에서 문구가 읽히는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **공지 제목 초과 토스트** | 모임 > 모임 상세 > 공지 작성/수정 > 제목 입력칸 | 제목 내부 스크롤은 유지되고, 글자수 초과 토스트가 모달 위에 정상 노출되는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **공지 내용 초과 토스트** | 모임 > 모임 상세 > 공지 작성/수정 > 내용 입력칸 | 내용 내부 스크롤은 유지되고, 글자수 초과 토스트가 모달 위에 정상 노출되는지 확인. | 2026-06-17 | 2026-06-17 |

...
| ⬜ | **공지사항 댓글 등록 버튼 문구/크기/키보드** | 모임 상세 → 공지사항 → 공지 상세 → 댓글 입력 영역 | 댓글 작성 버튼이 `입력`이 아니라 `등록`으로 표시되고, 버튼 크기/간격/입력창 높이가 책이야기 댓글 입력 영역과 동일한 기준으로 보임. 키보드가 열린 상태에서 `등록`을 누르면 첫 터치에 댓글이 바로 등록되고 키보드도 내려감. 댓글 수정 상태는 기존처럼 `수정`으로 표시됨. | 2026-07-06 | 2026-07-06 |

...
| 8 | `## 🔤 글자수 테스트 육안` | 중하단 | 입력 글자수 제한/스크롤/토스트 실기기 확인 |
| 9 | `## 👀 버그수정 육안 확인` | 중하단 | 버그수정/기능변경 후 실기기 육안 검증 체크리스트 |
| 10 | `## 🐛 issue-fetch 미완료 항목` | 하단 | API/계약 이슈 추적 항목 |
| 11 | `## 🗒 2026-06-18 QA 수집 (상세 설명 대기)` | issue-fetch 아래 | 사용자 QA 메모 원문 수집, 상세 설명 대기 |
| 12 | `## 상태 범례` | 문서 최하단 | 상태 표기 기준 |
...
| ⬜ | **POST 후 즉시 반영 누락 정리** | 뮤테이션 성공 후 화면에 즉시 반영 안 되는 케이스 수정(낙관적 업데이트 or 성공 후 재조회 추가). 대상 ❌ 5건: ①모임 가입신청 `joinClub`(`MeetingScreen.tsx:753`) → 내 모임 목록/멤버십 미갱신, ②모임 삭제 `deleteClub`(`useManagementState.ts:520`) → 이전 화면 내 모임 목록에 삭제 모임 잔존(404 위험), ③차단 `blockMember`(`UserProfileScreen.tsx:524`) → 차단 상태/콘텐츠 숨김 미반영, ④신고 `createReport`(`UserProfileScreen.tsx:588`) → 마이페이지 신고관리 히스토리 미갱신, ⑤이메일 변경 `updateMyEmail`(`useAccountSettingsState.ts:169`) → 마이페이지 표시 이메일 미갱신. ⚠️ 추가: 책 좋아요 화면 간 동기화(헤더/홈/서재 독립 state라 재방문 전 미반영), 책이야기 삭제 `deleteBookStory`(`StoryScreen.tsx:1033`) 실패 시 롤백 없음. DoD: 각 동작 성공 직후 해당 화면/연관 목록에 변경이 보이고, 실패 시 롤백 또는 안내. | 2026-06-22 | 2026-06-22 |
| ⬜ | **웹 애플로그인 키 발급** | Apple Developer Console에서 웹용 Sign in with Apple 자격증명을 발급한다. 무엇을: Services ID(클라이언트 식별자) 생성, Sign in with Apple용 Key(.p8) 발급(Key ID 확보), Team ID 확인, Return URL/도메인(웹 OAuth2 redirect) 등록. 왜: BE는 웹 OAuth2(redirect) 방식이라 애플 웹 로그인 연동에 Services ID·Key·Team ID·Key ID가 필요. DoD: Services ID/Key(.p8)/Key ID/Team ID 확보 및 redirect 도메인 등록 완료, BE 환경변수에 주입 가능한 상태. 참고: `docs/documents/apple-login-backend-plan.md` | 2026-06-21 | 2026-06-21 |
| ⬜ | **온보딩(첫 사용자 가이드) 추가** | 처음 진입한 사용자가 5개 핵심 화면을 스와이프로 훑는 온보딩 캐러셀 추가. 확정안: 5장(홈+헤더/모임/책이야기/소식/마이페이지), 하이브리드 비주얼(실제 스샷+콜아웃), 첫 콜드런치 즉시 노출(로그인 무관)·건너뛰기·마이페이지 다시보기. 어디서: `App.tsx` 노출 게이트, 신규 `OnboardingScreen`/`onboardingSlides`/`onboardingStore`(SecureStore 버전키). DoD: 신규 설치 첫 실행 1회 노출 후 재실행 시 미노출, 다시보기 동작, GUEST 노출, `npm run check` 통과. 참고: `docs/documents/onboarding-plan.md` | 2026-06-21 | 2026-06-21 |
...
| ⬜ | **공지사항 리스트 로딩 문제** | MeetingScreen 모임 내부 공지사항 탭에서 리스트 로딩/갱신 상태가 비정상인 케이스를 재현하고 수정한다. 어디서: `GroupNoticeView`, `useNoticeState`, workspace reload 흐름. 무엇을: 초기 진입/탭 전환/새로고침/공지 작성·수정·삭제 후 공지 리스트가 비거나 늦게 반영되는 문제 확인. 왜: 공지사항은 모임 핵심 정보라 로딩 실패나 빈 화면 오인 가능성이 큼. DoD: 공지 리스트가 초기 진입과 갱신 후 항상 정상 표시되고, 로딩/빈 목록/오류 상태가 구분되어 보임. | 2026-06-14 | 2026-06-14 |
...
| ⬜ | **모임 생성 로직 수정** | MeetingScreen 모임 생성 플로우에서 입력값 검증, 이미지/링크 포함 payload, 생성 후 화면 전환 로직을 재검토한다. DoD: 필수/선택 입력 조합별 생성 성공 및 실패 안내가 의도대로 동작. | 2026-06-13 | 2026-06-13 |
...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **조 관리 드래그 자동 스크롤 QA** | 모임 → 책장 → 정기모임 → 조 관리하기 | 하단 회원을 상단에 유지하면 1조까지, 상단 회원을 하단에 유지하면 미배정까지 연속 이동하는지 확인. 상단 빠른 조 칩/본문 조 카드 드롭, 활성 테두리, 경계 정지, 드래그 취소 시 원래 조 유지, 탭 선택 후 조 버튼 이동을 iOS·Android에서 확인. | 2026-06-22 | 2026-06-22 |
| ⬜ | **조별 채팅 재도입 실기기 QA** | iOS/Android 두 계정 → 모임 → 책장 → 정기모임 → 조 상세 → 채팅 FAB | 일반 회원은 소속 조만·운영진은 전체 조가 보이는지, 최근 30개/상단 과거 로드 위치 유지, 두 계정 실시간 송수신·중복 방지·재연결을 확인. 작성자 클릭 시 인라인 프로필 모달(사진/아이디/사용자 신고/바로가기), 내 채팅 박스는 복사 메뉴, 타인 박스는 복사/신고 메뉴가 열리고 실제 클립보드 값과 신고 messageId가 맞는지 확인. 채팅 전용 `⋮` 제거, 키보드·safe-area·하드웨어/스와이프 뒤로가기도 확인. | 2026-06-22 | 2026-06-22 |
...
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **책이야기 본문 긴 입력** | 책 이야기 > 책이야기 작성/수정 > 본문 입력칸 | 전수 조사 문서(`docs/documents/long-input-field-audit.md`) 작성 및 RN 입력칸 내부 스크롤/하단 여백 보정 완료. BE는 본문 길이 제한 없음(`TEXT`), RN은 5000자 제한으로 불일치 문서화. iOS·Android에서 긴 본문 입력 시 내부 스크롤, 마지막 줄/커서, 카운터, 초과 토스트가 가려지지 않는지 확인 필요. | 2026-06-17 | 2026-06-25 |
| ⬜ | **신고 내용 긴 입력/초과 토스트** | 프로필/사용자/댓글 메뉴 등 > 신고하기 > 신고 내용 입력칸 | BE/RN 500자 일치 확인, 카운터/초과 clamp+토스트 연결 확인, RN 입력칸 maxHeight/하단 여백 보정 완료. iOS·Android에서 긴 신고 내용 입력/붙여넣기 시 내부 스크롤과 토스트가 키보드에 가리지 않는지 확인 필요. | 2026-06-17 | 2026-06-25 |
| ⬜ | **모임 정보 수정 소개글 초과 토스트** | 모임 > 모임 상세 > 관리 > 모임 정보 수정 > 모임 소개글 입력칸 | BE/RN 500자 일치 확인, 모임 생성/수정 양쪽 카운터/초과 clamp+토스트 연결 확인, 공통 textArea minHeight/하단 여백 보정 완료. iOS·Android에서 소개글 초과 토스트와 마지막 줄/커서 가시성 확인 필요. | 2026-06-17 | 2026-06-25 |
| ⬜ | **발제·한줄평 초과 토스트 밝기** | 모임 > 모임 상세 > 책장/발제·한줄평 작성 > 발제 내용 또는 한줄평 내용 입력칸 | BE/RN 300자 일치 확인, 공용 composer 카운터/초과 clamp+토스트 연결 확인, 입력칸 minHeight/내부 스크롤/하단 여백 보정 완료. iOS·Android에서 발제·한줄평 각각 초과 토스트와 내부 스크롤 확인 필요. | 2026-06-17 | 2026-06-25 |
| ⬜ | **공지 제목 초과 토스트** | 모임 > 모임 상세 > 공지 작성/수정 > 제목 입력칸 | RN 입력칸 내부 스크롤/하단 여백 보정 완료. BE 40자/RN 50자 불일치는 문서화만 하고 값은 유지. iOS·Android에서 제목 초과 토스트, 내부 스크롤, 카운터 가시성 확인 필요. | 2026-06-17 | 2026-06-25 |
| ⬜ | **공지 내용 초과 토스트** | 모임 > 모임 상세 > 공지 작성/수정 > 내용 입력칸 | RN 입력칸 내부 스크롤/하단 여백 보정 완료. BE 1000자/RN 2000자 불일치는 문서화만 하고 값은 유지. iOS·Android에서 내용 초과 토스트, 부모 스크롤 충돌 여부, 마지막 줄/커서 가시성 확인 필요. | 2026-06-17 | 2026-06-25 |
...
| 상태 | 항목 | 어디서 | 뭘 확인하나 | 생성일자 | 최종 편집일자 |
| ------ | ------ | ------ | ------ | ------ | ------ |
| ⬜ | **모임 가입신청 입력칸 키보드 포커싱** | 모임 > 모임 카드 > 가입신청하기 > 신청 사유 입력칸 | 가입신청하기 클릭 시 신청 사유 입력칸이 자동 포커스되고, 입력칸 전체가 키보드 위에 보여 가려지지 않는지 확인. | 2026-06-17 | 2026-06-17 |
| ⬜ | **공개모임 가입 즉시 반영 QA** | 모임 > 공개모임 카드 > 가입신청하기 | 공개모임 가입 성공 직후 카드 문구가 `가입 완료되었습니다`로 바뀌고, 새로고침 없이 내 모임 목록에 나타나는지 확인. 같은 카드/상세 진입 시 신청 완료가 아니라 가입 완료 상태인지 확인. 비공개모임은 `신청 완료되었습니다`로 남는지 함께 확인. | 2026-06-26 | 2026-06-26 |
| ⬜ | **모임 생성·수정·삭제 목록 즉시 반영 QA** | 모임 > 모임 생성 / 모임 상세 > 운영진 관리 > 모임 수정·삭제 | 모임 생성 후 내 모임 목록에 새 모임이 나타나는지, 수정 후 상세/내 모임/검색 카드 표시가 어긋나지 않는지, 삭제 후 상세가 닫히고 목록에서 사라지며 재진입 404가 없는지 확인. | 2026-06-26 | 2026-06-26 |
| ⬜ | **책 좋아요 화면 간 동기화 QA** | 헤더 책 검색 / 마이페이지 > 내 서재 | 헤더 검색 결과·책 상세에서 내 서재 담기/제거 시 마이페이지 내 서재가 즉시 추가/삭제되는지, 마이페이지에서 좋아요 취소 시 카드가 사라지고 헤더 하트 상태도 같이 바뀌는지 확인. | 2026-06-26 | 2026-06-26 |
...
| ✅ | **공지 작성/수정 입력·투표 QA** | 모임 → 관리 → 공지 작성/수정 | 실기기 통과 확인(2026-06-14). 투표 날짜 오프셋 없는 `LocalDateTime` 전송으로 투표 있는 공지 등록/수정 정상, 투표 기간 네이티브 datepicker, 투표 항목 최대 6개(6개면 "항목 추가" 숨김), "투표가 있는 공지사항은 수정이 불가합니다" 빨간 문구, 제목/내용 입력 진동 없음, 제목 50자/내용 2000자 제한·토스트, 투표 항목 3번부터 X 삭제 모두 정상. | 2026-06-14 | 2026-06-14 |
| ✅ | **공지 내용 글자수 카운터** | 모임 → 관리 → 공지 작성 | 실기기 통과 확인. 내용 카운터 위치/실시간 갱신, 2000자 제한/토스트, 내부 스크롤 및 커서 이탈 없음 정상. | 2026-06-14 | 2026-06-14 |
...
| ------ | ------ | ------ | ------ | ------ |
| ⬜ | **모임 생성 안됨** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ⬜ | **소식 날짜 이상하게 나옴** | 바로 바로 반영 되게. 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ⬜ | **모임 사람들 보고 싶음** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ⬜ | **알림 빨간색 유효 페이지 다름** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ⬜ | **댓글 최대 개수 막아놓기** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ✅ | **신고하기 안됨** | 사용자 실기기 통과 확인. | 2026-06-18 | 2026-06-26 |
| ⬜ | **공지사항 댓글 placeholder** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |
| ⬜ | **알림 이동 안됨** | 상세 설명 대기 | 2026-06-18 | 2026-06-18 |

...
| ⬜ | **모임 회원 관리 본인 탈퇴/로그인 유지** | 모임 상세 → 모임 관리하기 → 모임 회원 관리 → 내 계정 `역할 수정` → `회원 탈퇴` | 내 계정을 탈퇴 처리하면 마이페이지 내 모임 탈퇴와 같은 API로 탈퇴되고, 성공 후 책모 홈 탭으로 이동함. 탈퇴 직후 로그인이 풀리지 않고 다른 로그인 필요 화면/API도 정상 동작함. 타인 회원 탈퇴는 기존처럼 운영진 제외 처리로 동작함. | 2026-07-06 | 2026-07-06 |

...
| ------ | ------ | ------ | ------ | ------ |
| 🔄 | **책 검색 체감 속도 단축** | 서버 알라딘 의존(캐시미스 250~980ms)이라 클라에서 단축. 공용 훅 `useBookSearch`(debounce 400ms prefetch + 최소2자 + normalize/dedupe + AbortController + TTL 캐시 3분). Phase 0(인프라)·Phase 1(헤더)·Phase 2(책이야기·책장 검색 훅 재사용) **완료** — 3곳 모두 `useBookSearch`로 통일. Phase 3(`totalResults` 표기)만 남음. 참고: `docs/documents/book-search-latency-plan.md` | 2026-06-22 | 2026-06-22 |

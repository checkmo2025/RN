...
9) 알맞은 섹션이 없으면 새 섹션을 만든다.
- 현재 섹션 목록 중 어느 곳에도 맞지 않으면 새로운 `##` 섹션을 생성한다.
- 새 섹션을 만들었으면 바로 아래 `섹션 목록`에도 같은 턴에 추가한다.
- 중복 섹션명이 생기지 않게 기존 유사 섹션을 먼저 확인한다.
```
...
| 9 | `## 🔧 아이콘 교체 작업` | 하단 | 아이콘 교체 가이드/체크리스트 |
...
| ⬜ | **[TOKEN-03] zIndex 레이어 스케일 토큰화** | `layers.base/sticky/dropdown/overlay/toast` 5단계 상수 정의 후 9곳 하드코딩 교체. `zIndex: 999` 제거. 완료 기준(DoD): 임의 zIndex 숫자 0건. 참고: `docs/documents/ui-interaction-token-consistency.md` 2-3. | 2026-05-06 | 2026-05-06 |
| ⬜ | **[TOKEN-04] hitSlop 기준 통일** | hitSlop 6 → 8 통일(35건 중 hitSlop=6인 4건 교체). `touchTarget.hitSlop.icon/inline = 8` 상수화. 완료 기준(DoD): `hitSlop={6}` 0건. 참고: `docs/documents/ui-interaction-token-consistency.md` 2-4. | 2026-05-06 | 2026-05-06 |
| ⬜ | **[TOKEN-05] 버튼 height 4단계 고정** | `buttonSize.chip=28/icon=36/field=48/cta=52` 토큰 정의. MeetingScreen/MyPageScreen 버튼 규격 수렴. 완료 기준(DoD): height 13종 → 4종으로 수렴. 참고: `docs/documents/ui-interaction-token-consistency.md` 2-5. | 2026-05-06 | 2026-05-06 |

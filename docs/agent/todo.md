...
| ------ | ------ | ------ | ------ | ------ |
| 🔄 | **책 검색 체감 속도 단축** | 서버 알라딘 의존(캐시미스 250~980ms)이라 클라에서 단축. 공용 훅 `useBookSearch`(debounce 400ms prefetch + 최소2자 + normalize/dedupe + AbortController + TTL 캐시 3분). Phase 0(인프라: `requestJson` 외부 signal 연결, `searchBooks {signal}`)·Phase 1(헤더 `AppHeader` 적용) **완료**. Phase 2(책이야기·책장 검색 훅 재사용) / Phase 3(`totalResults` 표기) 남음. 참고: `docs/documents/book-search-latency-plan.md` | 2026-06-22 | 2026-06-22 |
...
| ✅ | **BE 문서** | `CHAT-03` | 채팅 히스토리 API 권한 응답 문서 누락 | 채팅 기능 전면 제거로 N/A. | - | 2026-06-04 |

...
> 생성일: 2026-07-10 KST
> 출처: `docs/agent/todo.md`
> 성격: 현재 TODO에서 잘라낸 완료/통과 항목 로그입니다.
> 운영 규칙: 에이전트는 기본 작업 시 이 문서를 필수로 읽지 않습니다. 사용자가 과거 완료 항목 확인을 명시할 때만 참고합니다.
...
| ------ | ------ | ------ | ------ | ------ |
| ✅ | **온보딩(첫 사용자 가이드) 추가** | 제공된 PNG 5장 기반 온보딩 캐러셀 구현 완료. 첫 앱 준비 완료 후 1회 노출하고, 하단 `건너뛰고 시작하기`/`책모 시작하기` 버튼으로 종료 시 `SecureStore`에 본 여부를 저장한다. 앱 업데이트 게이트와 겹치지 않도록 버전 상태가 정상일 때만 노출. DoD: `npm run check` 통과. | 2026-06-21 | 2026-07-09 |
| ✅ | **플레이스토어 제출 전 사용자 준비** | 사용자 통과 확인(2026-07-09). Google Play Console 코드 외 제출 준비 항목 완료 처리. 개발자 계정/결제 프로필, 앱 생성(`kr.co.checkmo.app`), 앱 메타데이터/그래픽/스크린샷, 개인정보처리방침 URL, 데이터 보안·콘텐츠 등급·타겟층·광고 여부·앱 액세스 권한·뉴스 앱 여부 등 정책 설문, 서비스 계정 JSON 및 내부 테스트 트랙 제출 기준 확인 완료. | 2026-06-16 | 2026-07-09 |
...
| ✅ | **책 검색 체감 속도 단축** | 서버 알라딘 의존(캐시미스 250~980ms)이라 클라에서 단축. 공용 훅 `useBookSearch`(debounce 400ms prefetch + 최소2자 + normalize/dedupe + AbortController + TTL 캐시 3분). Phase 0(인프라)·1(헤더)·2(책이야기·책장 통일)·3(`totalResults` 정확 표기) **전부 완료** — 헤더/책이야기/책장 3곳 단일화. 참고: `docs/archive/(done)book-search-latency-plan.md` | 2026-06-22 | 2026-06-22 |
...
| ✅ | **웹 애플로그인 키 발급** | 사용자 통과 확인(2026-06-27). Apple Developer Console 웹용 Sign in with Apple 자격증명 발급 항목 완료 처리. DoD: Services ID/Key(.p8)/Key ID/Team ID 확보 및 redirect 도메인 등록 완료, BE 환경변수에 주입 가능한 상태. 참고: `docs/archive/(done)apple-login-backend-plan.md` | 2026-06-21 | 2026-06-27 |
...
| ✅ | **플레이스토어 Android 실기기 QA** | Android 폰/태블릿 portrait, 가능하면 내부 테스트 트랙 설치본 | 사용자 실기기 통과 확인(2026-07-09). 회원가입/로그인/프로필 이미지 선택/홈/모임/책이야기/마이페이지 핵심 플로우 정상. 프로필 이미지 선택 시 불필요한 마이크/카메라/전체 사진 접근 권한 팝업 없이 시스템 picker가 열리고, 태블릿 텍스트 잘림·모달 폭·바텀시트 safe-area·이미지 뷰어 이상 없음. | 2026-06-16 | 2026-07-09 |
...
| ✅ | **뒤로가기 액션 확인** | 책이야기 상세 / 모임 상세 / 마이페이지 설정 / 회원가입 각 단계 | 사용자 실기기 통과 확인(2026-07-09). 하드웨어 뒤로가기(Android) 및 스와이프 백(iOS)이 각 화면에서 의도한 화면으로 이동하고, 작성 중 내용이 있을 때 이탈 경고가 뜨며, 모달 열린 상태에서는 뒤로가기 시 모달만 닫힘. | 2026-06-14 | 2026-07-09 |
...
| ✅ | **모임 채팅 웹소켓 배포 앱 동작** | 모임 → 책장 → 정기모임 → 조 상세 → 채팅 FAB | 사용자 배포 앱 통과 확인(2026-07-09). 조 채팅 접속, 최근 메시지 로드, 실시간 송수신, 재연결, 과거 메시지 더 불러오기 정상. 인증/권한 오류 반복 없음. | 2026-07-03 | 2026-07-09 |
...
| ✅ | **앱 로그인 세션 유지** | 로그인한 상태 → 앱 백그라운드/재실행/약 1시간 후 재접속 | 사용자 실기기 통과 확인(2026-07-09). 로그인 상태가 의도치 않게 풀리지 않고, 앱 로그인 경로에서 refresh/restore 후 홈·마이페이지·인증 필요 API가 정상 동작함. | 2026-07-03 | 2026-07-09 |
...
| ✅ | **생성 시간 상대표기 갱신** | 알림 목록, 책이야기 목록/상세/댓글 등 `방금 전` 표시 화면 | 코드상 통과 확인(2026-07-06). `LocalDateTime`을 KST 벽시계 시간으로 파싱하고, `useRelativeNow()`가 30초 주기/앱 active 복귀마다 현재 시각을 갱신해 책이야기·알림·댓글 상대표기가 재계산됨. | 2026-07-03 | 2026-07-06 |
| ✅ | **마이페이지 신고 관리 매핑** | 마이페이지 → 설정 → 신고 관리 | 사용자 실기기 통과 확인(2026-07-06). 내가 신고한 대상 목록이 비어 있지 않게 표시되고, 신고 대상/사유/날짜가 올바르게 매핑됨. API 실패가 `신고한 내역이 없습니다.`로 숨겨지지 않고 오류 안내로 보이며, 신고 대상 프로필/콘텐츠 진입이 가능한 경우 정상 이동함. | 2026-07-03 | 2026-07-06 |
...
| ✅ | **마이페이지 프로필 편집 키보드/개행** | 마이페이지 → 프로필 편집 | 사용자 실기기 통과 확인(2026-07-06). 키보드가 열린 상태에서도 닉네임 중복확인/저장 버튼이 터치되고, 소개글 입력에서 엔터 개행이 가능하며 텍스트/하단 카운터가 잘리지 않음. | 2026-07-03 | 2026-07-06 |
| ✅ | **Android 공지 투표 옵션 터치/상단 여백** | Android → 모임 상세 → 운영진 관리 → 공지 작성 → 투표 | 코드상 반영 기준 통과 처리(2026-07-06). `실명`과 `중복 가능` 칩이 키보드 상태에서도 선택되고, 공지 작성 화면이 처음부터 아래로 밀려 보이지 않음. 투표 저장 payload가 의도한 옵션으로 반영됨. | 2026-07-03 | 2026-07-06 |
...
| ✅ | **신고 작성 중 닫기 확인창** | 모임 상세/공지/댓글/발제/채팅 등 → 신고하기 → 내용 입력 후 `X`/바깥 영역/Android 뒤로가기 | 사용자 실기기 통과 확인(2026-07-06). 신고 내용을 입력한 상태에서 닫으려 하면 `현재 페이지는 저장되지 않습니다.` 확인창이 뜨고, `취소`는 신고 모달 유지, `닫기`는 모달 닫힘. 내용이 비어 있으면 기존처럼 바로 닫힘. | 2026-07-06 | 2026-07-06 |
| ✅ | **신고하기 접수/신고 관리 반영** | 책이야기/댓글/모임/공지/발제/채팅 등 → 신고하기 → 마이페이지 → 신고 관리 | 사용자 실기기 통과 확인(2026-07-06). 신고 등록 성공 토스트가 뜨고, 마이페이지 신고 관리에 방금 신고한 대상/사유/날짜가 표시됨. 대상이 삭제됐거나 BE가 표시 정보를 만들지 못하는 경우 빈 목록으로 숨기지 않고 오류 안내가 보임. | 2026-07-06 | 2026-07-06 |
| ✅ | **모임 회원 관리 본인 탈퇴/로그인 유지** | 모임 상세 → 모임 관리하기 → 모임 회원 관리 → 내 계정 `역할 수정` → `회원 탈퇴` | 사용자 실기기 통과 확인(2026-07-06). 내 계정을 탈퇴 처리하면 마이페이지 내 모임 탈퇴와 같은 API로 탈퇴되고, 성공 후 책모 홈 탭으로 이동함. 탈퇴 직후 로그인이 풀리지 않고 다른 로그인 필요 화면/API도 정상 동작함. 타인 회원 탈퇴는 기존처럼 운영진 제외 처리로 동작함. | 2026-07-06 | 2026-07-06 |
| ✅ | **모임 가입 거절 후 재신청 버튼 활성화** | 모임 가입 신청 → 운영진이 거절 → 신청자 계정으로 모임 탭 복귀 | 코드상 반영 기준 통과 처리(2026-07-06). `REJECT`/미가입 상태는 `applicationStatus`로 매핑하지 않고, 모임 탭 focus/새로고침으로 서버 상태를 다시 받으면 로컬 `신청완료` 표시를 제거해 `가입 신청하기` 버튼이 다시 활성화됨. | 2026-07-06 | 2026-07-06 |
...
| ✅ | **책이야기 댓글 등록 버튼 문구** | 책이야기 상세 → 댓글/대댓글 입력 영역 | 사용자 실기기 통과 확인(2026-07-06). 일반 댓글과 대댓글 작성 버튼이 `입력`이 아니라 `등록`으로 표시됨. 댓글 수정 상태는 기존처럼 `수정`으로 표시됨. | 2026-07-06 | 2026-07-06 |
| ✅ | **공지사항 댓글 등록 버튼 문구/크기/키보드** | 모임 상세 → 공지사항 → 공지 상세 → 댓글 입력 영역 | 사용자 실기기 통과 확인(2026-07-06). 댓글 작성 버튼이 `입력`이 아니라 `등록`으로 표시되고, 버튼 크기/간격/입력창 높이가 책이야기 댓글 입력 영역과 동일한 기준으로 보임. 키보드가 열린 상태에서 `등록`을 누르면 첫 터치에 댓글이 바로 등록되고 키보드도 내려감. 댓글 수정 상태는 기존처럼 `수정`으로 표시됨. | 2026-07-06 | 2026-07-06 |
| ✅ | **한줄평 별점 드래그 입력** | 모임 → 책장 → 한줄평 추가/수정 → 평점 | 사용자 실기기 통과 확인(2026-07-09). 별점 영역 탭/좌우 드래그 시 0.5점 단위로 자연스럽게 변경되고, 세로 스크롤 방해 없이 표시 별/점수 텍스트/등록 payload가 같은 값으로 유지됨. | 2026-07-06 | 2026-07-09 |
| ✅ | **입력 후 버튼 클릭 시 키보드 닫힘/즉시 실행 전수 확인** | 책이야기 댓글·작성·책 검색, 공지 댓글·작성, 모임 가입신청·생성·정보수정·책장, 마이페이지 프로필/계정 변경, 로그인/회원가입/찾기, 공통 책 검색, 채팅/신고 | 사용자 실기기 통과 확인(2026-07-06). 키보드가 열린 상태에서 등록/저장/검색/중복확인/다음/완료/전송 버튼을 누르면 첫 터치에 기능이 즉시 실행되고 키보드도 내려감. 버튼 동작 없이 키보드만 닫히거나, 기능은 실행됐는데 키보드가 계속 남는 화면이 없어야 함. 채팅 전송은 이번 통일 기준에 따라 전송 후 키보드가 내려감. | 2026-07-06 | 2026-07-06 |
| ✅ | **책 검색 게스트 인증 모달 통일** | 비로그인 상태 → 책 검색 → 검색 결과/상세/추천 책의 하트 또는 글 작성 버튼 | 사용자 실기기 통과 확인(2026-07-06). 하트 클릭 시 토스트만 뜨지 않고 로그인 모달이 열림. 검색 2뎁스 결과/상세에서 로그인 `X`를 누르면 직전 검색 화면으로 돌아오고, 글 작성 버튼도 책이야기 탭으로 먼저 튕기지 않고 현재 책 검색 화면 위에서 로그인 모달이 열림. 로그인 성공 시 하트/글 작성 동작이 이어짐. | 2026-07-06 | 2026-07-06 |
...
| ✅ | **마이페이지 메인 breadcrumb 제거** | 마이페이지 메인 진입 | 사용자 실기기 통과 확인(2026-07-06). 프로필 영역 위에 `전체 > 마이페이지` breadcrumb가 보이지 않고, 프로필 사진/닉네임 영역이 자연스럽게 상단에 배치됨. 팔로워/팔로잉 등 내부 화면의 뒤로가기성 breadcrumb는 기존처럼 유지됨. | 2026-07-06 | 2026-07-06 |
| ✅ | **마이페이지 비로그인 프로필 스켈레톤** | 비로그인 상태 또는 세션 복구 직후 → 마이페이지 메인 | 코드상 반영 기준 통과 처리(2026-07-06). 프로필 API가 끝나기 전이나 비로그인 상태에서 `user_id`, 기본 소개글, `프로필을 불러오는 중...` 텍스트가 노출되지 않고 프로필 사진/닉네임/구독수/소개글 자리 전체가 스켈레톤으로 보임. 로그인된 상태에서는 프로필 로딩 완료 후 실제 닉네임/소개글/카테고리로 자연스럽게 전환됨. | 2026-07-06 | 2026-07-06 |
| ✅ | **마이페이지 내 책이야기 좋아요 하트 표시** | 마이페이지 → 내 책 이야기 탭 | 사용자 실기기 통과 확인(2026-07-06). API 응답에서 `likedByMe`/`liked`가 true인 내 책이야기 카드는 하트가 채워진 빨간 아이콘으로 보이고, false인 카드는 빈 하트로 보임. 새로고침/재진입 후에도 좋아요 개수와 하트 상태가 API 응답과 일치함. | 2026-07-06 | 2026-07-06 |
| ✅ | **모임 카드 클릭 가입신청 폼 열기** | 모임 탭 → 가입 가능한 모임 카드 본문 터치 | 사용자 실기기 통과 확인(2026-07-06). 모임 카드의 제목/이미지/본문 영역을 눌러도 `가입 신청하기` 버튼을 누른 것과 동일하게 신청 사유 입력칸이 열림. `방문하기` 버튼은 방문 동작만 실행되고, 이미 신청/가입 완료된 카드는 카드 본문을 눌러도 폼이 열리지 않음. | 2026-07-06 | 2026-07-06 |
| ✅ | **모임 조회 제목/긴 카드 텍스트 안정화** | 모임 탭 → 비로그인/로그인 기본 화면, 검색어 입력, 라디오/드롭다운 필터 변경, 긴 이름/지역 모임 카드 | 사용자 요청 기준 통과 처리(2026-07-06). 비로그인 기본 화면은 `전체 독서 모임`, 로그인 기본 화면은 `독서 모임 추천`, 검색/필터 상태는 `검색 결과` 제목이 항상 보임. 긴 모임명/대상/지역은 2줄 말줄임으로 카드와 버튼 영역을 밀거나 깨뜨리지 않음. | 2026-07-06 | 2026-07-06 |
| ✅ | **Android 발제 선택 카드 색상** | Android → 모임 → 책장 → 정기모임/조 상세 → 발제 카드 선택 상태 | 코드상 반영 기준 통과 처리(2026-07-06). 웹소켓으로 선택 상태가 바뀐 발제 카드가 iOS와 같은 브라운 계열 선택 배경/테두리/체크 색으로 보임. 미선택 카드는 기존 비활성 스타일을 유지하고, 다른 기기에서 선택해도 Android 화면에 같은 색상으로 즉시 반영됨. | 2026-07-06 | 2026-07-06 |
